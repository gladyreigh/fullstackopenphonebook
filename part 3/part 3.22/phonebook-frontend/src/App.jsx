...
const App = () => {
  ...
  const handleAddPerson = async () => {
    ...
    if (existingPerson) {
      if (window.confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)) {
        ...
        try {
          ...
          setErrorMessage(error.response?.data?.error || `Failed to update ${newName}'s number`);
          setTimeout(() => setErrorMessage(null), 5000);
        }
      }
      return;
    ...
    try {
      ...
      setErrorMessage(error.response?.data?.error || 'Failed to add new person');
      setTimeout(() => setErrorMessage(null), 5000);
      throw error; // Re-throw the error to be caught in PersonForm
    }
  };
